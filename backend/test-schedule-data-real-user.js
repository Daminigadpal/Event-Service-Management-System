...
// Test script with real user token
const testScheduleData = async () => {
  try {
    console.log('🔍 Testing Schedule Data Flow with Real User...');
    ...
    // Test the API endpoints with real user token
    const weekStart = new Date();
    weekStart.setDate(weekStart.getDate() - weekStart.getDay());
    const weekEnd = new Date(weekStart);
    weekEnd.setDate(weekEnd.getDate() + 6);
    
    console.log('\n🔍 Testing API endpoints with real user token...');
    ...
    const response = await fetch('http://localhost:5000/api/v1/staff-availability', {
      ...
      headers: {
        ...
        'Authorization': `Bearer ${user._id}` // Use user ID as mock token
      },
      body: JSON.stringify({
